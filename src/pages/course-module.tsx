...
// ── Code highlighting functions (exact copy from Paper) ──
const CODE_KEYWORDS = [
...
const CourseModulePage: React.FC = () => {
        ...
        const loadModule = async () => {
            try {
                const module = await import(
                    `../content/${courseSlug}/${moduleSlug}.md`
                ) as MarkdownModule;
                const processed = processMarkdownHtml(module.html);
                setContentHtml(processed);
    ...
    // Copy button handler (same delegation as Paper)
    useEffect(() => {
    ...
    if (!contentHtml) return (
        <div className="paper-error-screen">
            ...
            <Link to={`/blogs/courses/${courseSlug}`} className="home-backlink">← Back to course</Link>
        </div>
    );
    ...
    return (
        <main className="paper-isolated-wrapper">
            ...
            {/* Theme toggle (identical to Paper) */}
            <button
            ...
            <div className="paper-container-inner">
                <nav className="paper-nav-links">
                    ...
                    <Link to={`/blogs/courses/${courseSlug}`}>{courseSlug}</Link> /{' '}
                    <span className="active-nav-node">{meta.title || moduleSlug}</span>
                </nav>

                <h1 className="paper-title-outside">{meta.title || 'Module'}</h1>
                <div className="paper-author-title">{meta.author || 'Unknown Author'}</div>
                <div className="paper-post-meta">
                    Date: {meta.date || 'Unknown'} // Sector: /content/{courseSlug}/{moduleSlug}.md
                </div>
                ...
                <footer>
                    ...
                    <p className="paper-disclaimer">
                        This educational material is for personal learning only.
                    </p>
                </footer>
            </div>
        </main>
